        ...
        if len(tag_names) > 0:
        	with conn.cursor(cursor_factory=psycopg2.extras.NamedTupleCursor) as cur: 
        		...
        		for tag_id in tag_ids:
        			cur.execute("INSERT INTO sf_tagmap (tag_id, link_id) VALUES (%s, %s) ON CONFLICT DO NOTHING RETURNING tagmap_id", [tag_id, inserted_link_id])
        		cur.execute("SELECT sf_tag.name FROM sf_tag JOIN sf_tagmap ON sf_tag.tag_id = sf_tagmap.tag_id WHERE link_id=%s", [inserted_link_id])
